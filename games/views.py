...
def game_list(request):
    now = timezone.now()
    upcoming = Game.objects.select_related("home_team","away_team").filter(start_time_utc__gte=now).order_by("start_time_utc")[:50]
    recent = Game.objects.select_related("home_team","away_team").filter(start_time_utc__lt=now).order_by("-start_time_utc")[:20]
